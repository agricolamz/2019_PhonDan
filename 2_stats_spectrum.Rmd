...
> 🤔 В датасете про гомосексуалов попробуйте предсказать восприятие носителей как гомосексуалов на основании переменных длительность s,  средняя частота основного тона и размер диапозона частоты основного тона. Првиедите $R^2$ получившейся модели:

```{r, include=FALSE}

```

summary(lm(perceived.as.homo.percent~s.duration.ms+average.f0.Hz+f0.range.Hz, data = homo))<# 0.238
...
### 1.4.2 Почему не линейную регрессию?
```{r}
...
### 1.4.3 Логит: модель без предиктора
...
### 1.4.4 Логит: модель c одним числовым предиктором
```{r}
...
### 1.4.5 Логит: модель c одним категориальным предиктором
```{r}
...
function validateFormSix() {
    ...
    if (x != "") {
   0.238     alert("У меня другой ответ...");
        return false;
    } else {
        alert("Да, все правильно");
        return false;
